...
if (choice_SDSS_QSOs eq 'y') then begin
   ...
   errplot, s, (xis_q-poisson), (xis_q+poisson), thick=4.0, color=0 ;128
...
;   endif
   ...
   if choice_jaca08_model eq 'y' then begin
      ...
      oplot, r_model, xi_model, linestyle = 0, thick = 4
      oplot, findgen(101)/100.*(80.-40.)+40., ((findgen(101)/100.*(80.-40.)+40.)/7.35)^(-2.30)*(1.+2./3.*0.32+0.2*0.32*0.32), linestyle = 0, thick = 4
   endif
...
;        OP_20080120 has UNIFORM 0.30 < z < 2.90
   ...
   device, filename='xis_DR5_quasars_LARGE_scales_lin_temp.ps', $
           xsize=7.6, ysize=5.5, /inches, /color
   !p.multi=0
...
;        OP_20080120 has UNIFORM 0.30 < z < 2.90
   ...
   plot, s, xis_q, $
         ...
         xrange=[-100, 3000], yrange=[-0.0025, 0.01], $ 
         psym=4, $
         xstyle=1, ystyle=1, $
         charsize=2.2, charthick=3.2, $
         xtitle=' s / h!E-1!N Mpc', $
