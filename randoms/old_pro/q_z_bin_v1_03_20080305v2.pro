...
No_bin = 60                ;; redshift z=0->3 for 0.05 bins...
x       = fltarr(No_bin)
y       = fltarr(No_bin)
z_bin   = fltarr(No_bin)
z_bin_width = 0.05
...
if choice eq 'P' then begin
   ...
   random = mrdfits('ra_dec_randoms_npr.fits', 1)
   
   indx_PRI   = where( ((data.TS_T_QSO     eq 1) or $
                        (data.TS_T_HIZ     eq 1) or $
                        (data.TS_T_FIRST   eq 1)  and ( $
                        (data.z            le 2.90))),  N_TS_T_PRI)
   print, 'PRIMARYs le 2.90', N_TS_T_PRI 
   
   data_ra_pri         = data[indx_PRI].ra
   data_dec_pri        = data[indx_PRI].dec
   data_z_pri          = data[indx_PRI].z
   data_specObjID_prmTarget  = data[indx_PRI].specObjID

   openw, 11, 'DR5QSO_pri_data.dat'
   for i=0L, N_elements(data_z_pri)-1 do begin
...
if choice eq 'P' then begin
   plothist, sp[0:50000].z, bin=z_bin_width, /overplot, color=192
   plothist, random_z[50000:100000], bin=z_bin_width, /overplot, color=64
   xyouts, 2.1, 2000, 'Randoms', charsize=2.2, charthick=3, color=64
   xyouts, 2.1, 1900, "sp.z", charsize=2.2, charthick=3, color=192
