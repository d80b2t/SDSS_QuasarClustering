...
;;
;; Written by C. MacLeod, May 2015
;;

readpath  = './'
writepath = './'
;;
;; Magnitude measurements and selected data for SDSS DR7Q QSOs 
;; with at least two photometric observations.
;; http://www.astro.washington.edu/users/ivezic/cmacleod/qso_dr7/
;;
;; X = skip a column.
form='(D,F,F,F,F,F,'+$
...
set_plot, 'ps'
device,filename=writepath+'gpsf_dm_2obs_diffs_temp.eps', $
        xsize=10.0, ysize=8.0, $
        xoffset=0.2, yoffset=0.2, $
        /inches, /color, /encapsulated

plot, G_PSF, (G_PSF-G_PSF2), ps=3, $
      xrange=[14,27], yrange=[-2,2], $
      xstyle=1, ystyle=1, $
      charthick=charthick, charsize=charsize, $
      xtitle='!6 G_PSF', ytitle='(G_PSF - G_PSF2)', title='dm_2obs.dat'

device,/close
set_plot, 'X'     

;; QSOs with at least two photometric observations. The contents of
;; this file are taken from the SDSS DR7Q QSO Schneider et al. catalog...
readcol,readpath+'master_2obs.dat',f='X,D',ra,dec,skipline=127

;; SDSS STRIPE 82 spectroscopically confirmed quasars (N=9258) 
readcol,readpath+'DB_QSO_S82.dat', f='A,D,D,A,D',$
  dbID, ras, decs, SDR5ID, M_is, M_i_corr, redshifts82, mass_bh, lum_bol,$
...
;; (De-)selecting objects that don't have +ve g_psf mags
...
remove, here, g_psf2, g_psf,g_err,g_err2,s82flag,mjd_r,mjd_r2, SDR7ID,ra,dec,$
        dt, dm_u, dm_g, dm_r, dm_i, dm_z,dm_ge, redshifts

;; (De-)selecting objects that have g_psf mags > 0.15
...
oplot,     abs(dtspmax[gtzero]), abs(dmspmax[gtzero]), psym=8,color=250
oploterror,abs(tchg[choose]), abs(mchg[choose]), mechg[choose],psym=7,color=100,errcolor=100
device,filename=writepath+'dtrf_dg.eps'
plot,      abs(dt[s82all])     /(1.+redshifts[s82all]), abs(dm_g[s82all]), psym=3, ytit=textoidl('|\Deltag| (mag)'),xtit=textoidl('|\Deltat| (days)')
oploterror,abs(dt[s82all])     /(1.+redshifts[s82all]), abs(dm_g[s82all]), dm_ge[s82all],psym=3;,color=250,errcolor=250
oplot,     abs(dtspmax[gtzero])/(1.+redshift [gtzero]), abs(dmspmax[gtzero]), psym=8,color=250
oploterror,abs(tchg[choose])   /(1.+redshift [choose]), abs(mchg[choose]), mechg[choose],psym=7,color=100,errcolor=100
device,/close
device,/close

;;
;; biggish_DR10_new_trans_table.txt
;;
openw,3,writepath+'biggish_DR10_new_trans_table.txt'
printf,3,'# ID name ra dec z Delta(g) Err_Delta(g) Delta(t)'
for iobj=0,nboss-1 do begin
   printf,3, f='(A16,2x,A,2x,F10.6,2x,F10.6,2x,F5.3,2x,F5.2,2x,F5.2,2x,I4)',dbid[choose[iobj]],sdssjid[choose[iobj]],ra[choose[iobj]],dec[choose[iobj]],redshift[choose[iobj]],mchg[choose[iobj]],mechg[choose[iobj]],tchg[choose[iobj]]
endfor
close,3
...
readcol,readpath+'master_2obs.dat',f='A,D ,D ,D  ,'+$
