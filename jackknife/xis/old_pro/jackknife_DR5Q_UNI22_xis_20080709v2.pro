...
printf, 11, ' i,j,l, A_two, COV_sum, A_three '
Cov_sum = 0d
...
;     print, i,j, '  RR_ij ',  RR_ati, RR_atj, '  xis_LS  ',  xis_LS_ati, xis_LS_atj
...
;         ;; Appendix A
         ...
         RR_weight_ati = RR_sub[i,l]
         RR_weight_atj = RR_sub[j,l]
         
         xis_LS_sub_ati = xis_LS_sub[i,l]
         xis_LS_sub_atj = xis_LS_sub[j,l]
         
;         A_two = (sqrt(RR_weight_ati / RR_ati)) * (xis_LS_sub_ati - xis_LS_ati) * $
;                 (sqrt(RR_weight_atj / RR_atj)) * (xis_LS_sub_atj - xis_LS_atj) 
         
         A_two = (sqrt(RR_sub[i,l] / RR[i])) * (xis_LS_sub[i,l] - xis_LS[i]) * $
                 (sqrt(RR_sub[j,l] / RR[j])) * (xis_LS_sub[j,l] - xis_LS[j]) 
         
         
         if A_two eq 0.0 then begin
;            print,   i,j,l, RR[i], RR_sub[i,l], xis_LS[i], xis_LS_sub[i,l]
;            print,   i,j,l, RR[j], RR_sub[j,l], xis_LS[j], xis_LS_sub[j,l]
         endif
         
         A_three_i = (RR_weight_ati / RR_ati) * ((xis_LS_sub_ati - xis_LS_ati)^2)
         A_three_j = (RR_weight_atj / RR_atj) * ((xis_LS_sub_atj - xis_LS_atj)^2) 
         
         A_three_i = sqrt(A_three_i)
         A_three_j = sqrt(A_three_j)


         COV_sum = COV_sum + A_two
         
         printf, 10,  i, j, l, RR_weight_ati,  RR_weight_atj, RR_ati, RR_atj, $
                 (RR_weight_ati/RR_ati),  (RR_weight_atj/RR_atj), $
                 xis_LS_sub_ati, xis_LS_ati,  xis_LS_sub_atj, xis_LS_atj, $
                 format='(i4,i4,i4, i22,i22,i22,i22, d16.8,1x,d16.8,1x, d16.8,1x,d16.8,1x,d16.8,1x,d16.8,1x)'
         
         printf,11, i,j,l, A_two, COV_sum, A_three_i, A_three_j, $
;                (A_two/A_three), (COV_sum/A_three), $
                 format='(i4,i4,i4,  d16.8,1x, d16.8,1x, d16.8,1x, d16.8,1x)'

      endfor 
      COV(i,j) = COV_sum
;      REG(i,j) = COV_sum / A_three
