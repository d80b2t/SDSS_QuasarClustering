...
;; Can put in a s_max such as to limit the maximum 
;; scales that the COV matrix is calculated *and* inverted over...
w=where(s le 25., N)
RR = RR[w]
...
;; Again, can put in an s_max, this time so that
;; the below loop is sped up a little. 
;;
max_scale =25.0  ;; 100.0
...
;;   for jj = 0L, 300-1 do begin
   for jj = 0L, 30-1 do begin
      ...
      printf, 15, chi_sq,  s_nought, gamma, $
              format='(d16.8,2x, d16.8,2x, d16.8)'
...
!p.multi=0
set_plot, 'ps'
device, filename='REG_3D_temp.eps', $
...
set_plot, 'ps'
device, filename='xis_DR5_quasars_Poisson_vs_JK_errs_temp.eps', $
        xsize=7.5, ysize=6.0, $
        /encapsulated, $
