...
readcol, '../OP/OP_UNI22_jack_xis/k_output_UNI22_jack_N1.dat', $
         ...
         xis_LS_N1, xis_HAM_N1, ratio_N1, /silent
...
print, 'N11 DDs', sqrt(total(DD_N11)), '   RRs ', sqrt(total(RR_N11))
print, 'N12 DDs', sqrt(total(DD_N12)), '   RRs ', sqrt(total(RR_N12))
print, 'N13 DDs', sqrt(total(DD_N13)), '   RRs ', sqrt(total(RR_N13))
print, 'N14 DDs', sqrt(total(DD_N14)), '   RRs ', sqrt(total(RR_N14))
print, 'N15 DDs', sqrt(total(DD_N15)), '   RRs ', sqrt(total(RR_N15))
print, 'N16 DDs', sqrt(total(DD_N16)), '   RRs ', sqrt(total(RR_N16))
print, 'N17 DDs', sqrt(total(DD_N17)), '   RRs ', sqrt(total(RR_N17))
print, 'N18 DDs', sqrt(total(DD_N18)), '   RRs ', sqrt(total(RR_N18))
print, 'N19 DDs', sqrt(total(DD_N19)), '   RRs ', sqrt(total(RR_N19))
print, 'N20 DDs', sqrt(total(DD_N20)), '   RRs ', sqrt(total(RR_N20))
print, 'S01 DDs', sqrt(total(DD_S01)), '   RRs ', sqrt(total(RR_S01))
print, 'S02 DDs', sqrt(total(DD_S02)), '   RRs ', sqrt(total(RR_S02))
print, 'S03 DDs', sqrt(total(DD_S03)), '   RRs ', sqrt(total(RR_S03))
...
openw, 10, 'COV_ijl_temp.dat'
openw, 11, 'COV_buildup_temp.dat'
openw, 12, 'COV_temp.dat'
openw, 13, 'REG_temp.dat'
openw, 14, 'k_output_UNI22_jackknife_errors_temp.dat'
...
openw, 16, 'COV_matrix_temp.dat'
xx = indgen(n_elements(RR)) 
for i=0L, n_elements(RR)-1 do printf, 16, COV[xx,i], format='(39(d16.8,1x))'
close, 16

openw, 17, 'INV_COV_matrix_temp.dat'
xx = indgen(n_elements(RR)) 
for i=0L, n_elements(RR)-1 do printf, 17, inv_COV[xx,i], format='(39(d16.8,1x))'
close, 17



max_scale =25.0
w = where(s le max_scale, N)

openw, 15, 'Build_up_of_chi_sq_using_REG.dat'

chi_sq_min = 1000.
bla       = 100.

;for ii = 0L, 300-1 do begin
for ii = 0L, 30-1 do begin
   s_nought = 0.0 + (float(ii)*0.05)
   ;print, 's_nought', s_nought
;   for jj = 0L, 300-1 do begin
   for jj = 0L, 30-1 do begin
      gamma = 0.0 + (float(jj)*0.01)

      chi_sq =0.0
      for i=0L, N-1 do begin
         for j=0L, N-1 do begin
            
            model_xis_i = (s[i] / s_nought) ^ (-1.0 * gamma)
            model_xis_j = (s[j] / s_nought) ^ (-1.0 * gamma)

            chi_sq = chi_sq + $
                     (xis_LS[i] - model_xis_i) * $
                     inv_COV(i,j) * $
                     (xis_LS[j] - model_xis_j)
         endfor
      endfor
      ...
      if chi_sq lt chi_sq_min then begin
         ...
         jj_min = jj
         chi_sq_min = chi_sq
        ; print, s_nought, gamma, chi_sq_min
      endif

      printf, 15, chi_sq, bla, s_nought, gamma, $
              format='(d16.8,1x, f5.1,1x, d16.8,1x, d16.8)'
...
set_plot, 'ps'
device, filename='COV_3D_temp.ps', $
...
SURFACE, COV, $
         ...
         yrange=[0,13.], ystyle=1, $
         zrange=[0,16.], zstyle=1, $
         xtitle='!3s!D0,i / h!E-1!N Mpc', $
         xtickname=['1','2.2', '3.5','5.6', '8.9','14', '22'], $
         ytickformat='(a1)'
...
levels=16
step = (Max(COV) - Min(COV)) / levels
userLevels = IndGen(levels) * step + Min(COV)

!p.multi=0
set_plot, 'ps'
loadct, 0
device, filename='COV_2D_temp.ps', $
        xsize=8.3, ysize=6.0, /inches, /color, $
        xoffset=0.0, yoffset=0.2
CONTOUR, COV, $
         color=0, $
;         XRANGE=[0,13], YRANGE=[0,13], $
         position=[0.35, 0.14, 1.00, 1.00], $
         TITLE = 'Covariance Matrix, !7n!3(s)', $
         CHARSIZE = 2.2, charthick=4.2, thick=4.2, $
         Nlevels=255, $
;         LEVELS=[-1.0, -0.1, -0.01, -0.001, 0, 0.001, 0.01, 0.1, 1.0], $
         /fill, $
         c_colors=255, $
         c_linestyle=1., $
         xtickname=['1', '2', '4', '6', '10', '16', '25'], $
         ytickname=['1', '2', '4', '6', '10', '16', '25'], $
         xtitle='!3s!D0,i / h!E-1!N Mpc', $
         ytitle='!3s!D0,j / h!E-1!N Mpc'
...
label=string((scaley),format='(F4.2)')
for i = 0,255 do scale(*,i) = scaley[i]
loadct, 0
contour, scale, scalex, scaley, $
         position=[0.14,0.14,0.20,1.00], $ 
         /fill, $
;         /overplot, $
         nlev=255, $
         xstyle=1, ystyle=1, $
...
set_plot, 'ps'
device, filename='COV_3D_shade_temp.ps', $
        xsize=7.5, ysize=6.0, /inches, /color, xoffset=0.0, yoffset=0.2
...
!p.multi=0
set_plot, 'ps'
device, filename='REG_3D_temp.ps', $
        xsize=7.5, ysize=6.0, /inches, /color, xoffset=0.0, yoffset=0.2
SURFACE, REG, $ 
;         TITLE = 'Regression Matrix, !7n!3(s)', $
         /lego, $
         SKIRT=0.0, $
         xthick=2.0, ythick=2.0, thick=2.0, $
         color=0, $
...
;         AZ=-35, $ 
         ...
         xrange=[0,13.], xstyle=1, $
         yrange=[0,13.], ystyle=1, $
         zrange=[-1.0,1.0], zstyle=1, $
         xtitle='!3s!D0,i / h!E-1!N Mpc', $
         ytitle='!3s!D0,j / h!E-1!N Mpc', $
         xtickname=['1','2.2', '3.5','5.6', '8.9','14', '22'], $
         ytickname=['1','2.2', '3.5','5.6', '8.9','14', '22']
...
set_plot, 'ps'
loadct, 33
device, filename='REG_2D_temp.ps', $
        xsize=8.3, ysize=6.0, /inches, /color, $
        xoffset=0.0, yoffset=0.2
CONTOUR, REG, $
;         position=[0.14, 0.14, 0.80, 1.00], $
         position=[0.35, 0.14, 1.00, 1.00], $
;         /T3D, /NOERASE, $
         TITLE = 'Regression Matrix, !7n!3(s)', $
         CHARSIZE = 2.2, charthick=4.2, thick=4.2, $
;         levels=[-1.00, -0.50, 0.00, 0.50, 1.00], $
;         LEVELS = BYTSCL(INDGEN(8)), $
         LEVELS = ( INDGEN(9)/4. -1 ), $
         /XSTYLE, /YSTYLE, $  
         C_COLORS = BYTSCL(INDGEN(8)), $  
;         nlev=255, $
         /fill, $
         c_linestyle=1., $
;         xtickunits= , $
...
;         levels=[-1, -0.5, 0, 0.5, 1], $
         levels=[-1, -0.75, -0.5, -0.25, 0, 0.25, 0.5, 0.75, 1], $
         c_linestyle=0, c_color=0., $
;         c_annotation=['-1', '-0.5',  '0', '0.5', '1'], $
;         c_annotation=['-1','-0.75','-0.5','-0.25','0','0.25','0.5','0.75','1'], $
         /overplot
...
;      xrange=[0.99, 300], yrange=[0.001001, 500], $ ; bring down to
;      500 w/o V. PREL.
      xrange=[0.99, 300], yrange=[0.001001, 100], $ ; bring down to 500 w/o V. PREL.
      psym=8, $
      thick=4, $
      xstyle=1, ystyle=1, $
...
oplot, s, xis_LS, thick=4, color=4, linestyle=8*talk

oplot,  s_N1,  xis_LS_N1, linestyle=1, thick=4, color=12
oplot,  s_N2,  xis_LS_N2, linestyle=1, thick=4, color=24
oplot,  s_N3,  xis_LS_N3, linestyle=1, thick=4, color=36
oplot,  s_N4,  xis_LS_N4, linestyle=1, thick=4, color=48
oplot,  s_N5,  xis_LS_N5, linestyle=1, thick=4, color=60
oplot,  s_N6,  xis_LS_N6, linestyle=1, thick=4, color=72
oplot,  s_N7,  xis_LS_N7, linestyle=1, thick=4, color=84
oplot,  s_N8,  xis_LS_N8, linestyle=1, thick=4, color=96
oplot,  s_N9,  xis_LS_N9, linestyle=1, thick=4, color=108
oplot, s_N10, xis_LS_N10, linestyle=1, thick=4, color=120
oplot, s_N11, xis_LS_N11, linestyle=1, thick=4, color=132
oplot, s_N12, xis_LS_N12, linestyle=1, thick=4, color=144
oplot, s_N13, xis_LS_N13, linestyle=1, thick=4, color=156
oplot, s_N14, xis_LS_N14, linestyle=1, thick=4, color=168
oplot, s_N15, xis_LS_N15, linestyle=1, thick=4, color=180
oplot, s_N16, xis_LS_N16, linestyle=1, thick=4, color=192
oplot, s_N17, xis_LS_N17, linestyle=1, thick=4, color=204
oplot, s_N18, xis_LS_N18, linestyle=1, thick=4, color=216
oplot, s_N19, xis_LS_N19, linestyle=1, thick=4, color=228
oplot, s_N20, xis_LS_N20, linestyle=1, thick=4, color=240
oplot,   s_S, xis_LS_S,   linestyle=1, thick=4, color=256


legend, [''], psym=8, box=0, position=[10.,56.]   
xyouts, 15.0, 38.0, 'SDSS DR5Q (uni)', charsize=2.2, charthick=6.2, color=0
legend, [''], box=0, position=[6.,76.] ;, linestyle=0, thick=6   
xyouts, 15.0, 15.3,  '0.3 < z < 2.2 ', charsize=2.2, charthick=6.2, color=0
