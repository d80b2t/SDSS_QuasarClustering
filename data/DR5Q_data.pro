...
print
readcol, 'Richards_2006_Table4.dat', K_cor_redshift, K_cor, /silent
print

;; CORRECTING FOR M_i(z=2) for UNIFORMS, 
;; i.e. equation (1) of Richards et al. (2006):
;; M_i(z=0) = M_i(z=2) + 2.5(1 + alpha_nu)*log (1+2) 
;;          = M_i(z=2) + 0.596        if alpha_nu = -0.5.  AT REDSHIFT=0!!
;;        
;; NOTE: Using the data in e.g. Table 4 of Richards et al. (2006)
;;       gives you the full K-correction treatment. 
;;       Have to take out the continuum contribution to the
;;       K_correction, K_cont = -2.5(1+alpha_nu)log(1+z)
;;       to match up "data_full" and "data".

for i=0L, n_elements(data.z)-1 do begin
   K_cor_bin = fix(data[i].z /0.01)
   K_cor_offset = K_cor(K_cor_bin)
   data[i].imag = data[i].imag + (K_cor_offset-(-1.25*alog10(1+data[i].z)))
endfor
...
print, ' FULL DR5Q  ; FULL DR5Q   ; FULL DR5Q  ; FULL DR5Q  ; FULL DR5Q  ; FULL DR5Q' 
help, data_full
print, 'min  redshift (full DR5Q) = ', min(data_full.z)
print, 'max  redshift (full DR5Q) = ', max(data_full.z)
print, 'mean redshift (full DR5Q) = ', mean(data_full.z)
print, 'min Ab. i-Mag (full DR5Q) = ', min(data_full.M_I)
print, 'max Ab. i-Mag (full DR5Q) = ', max(data_full.M_I)
print
...
print, 'mean redshift (full DR5Q) = ', mean(data_full[indx_z_cut_full29].z)
...
print, 'mean redshift (full DR5Q) = ', mean(data_full[indx_z_cut_full03z29].z)
...
print, 'mean redshift (full DR5Q) = ', mean(data_full[indx_z_cut_full22].z)
...
print, 'mean redshift (full DR5Q) = ', mean(data_full[indx_z_cut_full03z22].z)
...
print, 'mean redshift (Shen) = ', mean(data.z)
...
print, 'mean redshift (UNIF, z<2.9) = ', mean(data[indx_z_cut29].z)
...
print, 'min  redshift (UNIF, z<2.2) = ', min(data[indx_z_cut22].z)
print, 'max  redshift (UNIF, z<2.2) = ', max(data[indx_z_cut22].z)
print, 'mean redshift (UNIF, z<2.2) = ', mean(data[indx_z_cut22].z)
...
print, 'min  redshift (UNIF, 0.3_<z_<2.9) = ', min(data[indx_z_cut03z29].z)
print, 'max  redshift (UNIF, 0.3_<z_<2.9) = ', max(data[indx_z_cut03z29].z)
print, 'mean  redshift (UNIF, 0.3_<z_<2.9) = ', mean(data[indx_z_cut03z29].z)
...
print, 'min  redshift (UNIF, 0.3_<z_<2.2) = ', min(data[indx_z_cut03z22].z)
print, 'max  redshift (UNIF, 0.3_<z_<2.2) = ', max(data[indx_z_cut03z22].z)
print, 'mean redshift (UNIF, 0.3_<z_<2.2) = ', mean(data[indx_z_cut03z22].z)
print, 'min Ab. i-Mag (Shen) = ', min(data[indx_z_cut03z22].imag)
print, 'max Ab. i-Mag (Shen) = ', max(data[indx_z_cut03z22].imag)
data_indx_z_cut03z22_imag = data[indx_z_cut03z22].imag 
data_indx_z_cut03z22_z    = data[indx_z_cut03z22].z
print
print
print
...
print, 'mean redshift (PRI) = ', mean(data_full[indx_PRI].z)
...
print, 'mean redshift (PRIzz) = ', mean(data_full[indx_PRIzz].z)
...
print, 'mean redshift (PRIz) = ', mean(data_full[indx_PRIz].z)
...
print, 'mean redshift (PRIzz22) = ', mean(data_full[indx_PRIzz22].z)
...
print, 'Starting PRI vs. UNI matching....'
openw, 9, 'PRIMARY_UNIFORM_RADec_diffs_temp.dat'
;for i=0L,n_elements(data.ra)-1 do begin
;for i=0L,n_elements(data_full[indx_PRI].ra)-1 do begin
;   diff = sqrt( (data_full[indx_PRI].ra-data[i].ra)^2 +
;   (data_full[indx_PRI].dec-data[i].dec)^2)
;   diff = sqrt( (data_full[indx_PRI[i]].ra-data.ra)^2 + (data_full[indx_PRI[i]].dec-data.dec)^2)
;   
;   w = where(diff eq min(diff), N)
;;   w = where(diff le (1./3600.), N)
;   
;   if N gt 1 then begin
;      print, data_full[i].ra, data_full[i].dec, data[w].ra, data[w].dec, diff[w], $
;                         format='(5(f16.10))'
;      w= w[0]
;   endif
;   
;   if w lt 0 then print, ' w lt zero !!!!', i;
;   if w gt 0 then printf, 9, data_full[indx_PRI[i]].ra, $ 
;                          data_full[indx_PRI[i]].dec, $ data[w].ra, data[w].dec, diff[w], $
;           format='(5(f16.10))'
   
;endfor
close, 9
print, 'DONE PRI vs. UNI matching.  '
...
!p.multi=0
set_plot, 'ps'
device, filename='SDSS_Quasar_Nofz.eps', $
        xsize=8, ysize=8, xoffset=0., $
       yoffset=0., /inches, /color, /encapsulated
plothist, data_full.z, bin=0.05, $
          xrange=[-0.05, 3.0], $
          yrange=[0.00, 2500], $
          position=[0.18,0.12,0.96,0.98], $
...
!p.font=-1
set_plot, 'ps'
device, filename='SDSS_Quasar_Ab_iMag_redshift.eps', $
        xsize=8, ysize=8, xoffset=0, $
...
;      yrange=[0,1800], $        ; for PRIz
      xstyle=1, ystyle=1, $
      xthick=4.2*talk, ythick=4.2*talk, thick=4.2*talk, $
      xtitle='redshift, z', $
;      XTICKFORMAT="(A1)", color=0, $
      xcharsize=2.2, charthick=4.2*talk, $
;      /nodata, $
      color=0

;plothist,  data_full[indx_PRIzz22].z, bin=0.05,  $
;           position=[0.32,0.12, 0.98,0.32], $
;           thick=4.2*talk, $
;           color=192, /overplot
;plothist,  data[indx_z_cut22].z, bin=0.05,  $
...
plot, data_full[indx_PSFMAG_I].z, data_full[indx_PSFMAG_I].M_I,   $
...
;oplot, data_full[indx_PRIzz22].z, data_full[indx_PRIzz22].M_I, psym=3, color=192
...
;;
;; Really quick and very dirty way (just for the sake of plotting!) 
;; to bring data and data_full lower M_i < -22 limits into line
w_lt22 = where(data_indx_z_cut03z22_imag le -22.000, N_lt_22)
data_imag = data_indx_z_cut03z22_imag[w_lt22]
data_z    = data_indx_z_cut03z22_z[w_lt22]
print, 'No. M_i < -22.00', N_lt_22

plotsym, 0, 0.4, /fill
;oplot, data.z, data.imag, psym=8,color=64 
;oplot, data[indx_z_cut03z22].z, data[indx_z_cut03z22].imag, psym=8, color=64 
oplot, data_z, data_imag, psym=8, color=64 
oplot, data_full.z, data_full.M_I,   psym=3,       color=0                    
;oplot, data_full[indx_PSFMAG_I].z, data_full[indx_PSFMAG_I].M_I, psym=3, color=0                    
...
;;
plothist, data_full[indx_PRIzz22].M_I, bin = 0.1, $
...
;      xrange=[0.0,0.49], yrange=[-21.5, -30.7], $
      ...
      xcharsize=0.8, ycharsize=2.2, charthick=4.2*talk, $
      xtickformat='(f3.1)', $
      ;ytickformat='(f3.1)', $
      ytitle='M!I i !N', $  
      position=[0.16,0.32,0.32,0.98], $
...
;plothist, data[indx_z_cut22].imag, bin = 0.1, $
...
xyouts, 1.7, -23.0,   '77 429  (dr5q)',  color=0,   charsize=2.2, charthick=6.2
;xyouts, 1.7, -22.5,   '46 272  (pri)',   color=192, charsize=2.2, charthick=6.2
xyouts, 1.7, -22.0,   '30 239  (uni)',   color=64,  charsize=2.2, charthick=6.2
...
;   if (data[i].z gt 0.999 and data[i].z lt 1.001) then printf, 10, data[i].sdss_name, data[i].ra, data[i].dec, data[i].z
      if (data[i].z gt 5.0 ) then printf, 10, data[i].sdss_name, data[i].ra, data[i].dec, data[i].z
endfor
print
close, 10
