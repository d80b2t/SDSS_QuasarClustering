...
print
;indx_z_cut_full25 = where(data_full.z le 2.50, N_z_cut)
;print, 'FULL, N_z_cut_full z _< 2.5', N_z_cut
;print, 'min  redshift (full DR5Q) = ', min(data_full[indx_z_cut_full25].z)
;print, 'max  redshift (full DR5Q) = ', max(data_full[indx_z_cut_full25].z)
;print, 'min Ab. i-Mag (full DR5Q) = ', min(data_full[indx_z_cut_full25].M_I)
;print, 'max Ab. i-Mag (full DR5Q) = ', max(data_full[indx_z_cut_full25].M_I)
;print
;indx_z_cut_full03z25 = where(data_full.z ge 0.30 and data_full.z le 2.50, N_z_cut)
;print, 'FULL, N_z_cut_full 0.30 _< z _< 2.5', N_z_cut
;print, 'min  redshift (full DR5Q) = ', min(data_full[indx_z_cut_full03z25].z)
;print, 'max  redshift (full DR5Q) = ', max(data_full[indx_z_cut_full03z25].z)
;print, 'min Ab. i-Mag (full DR5Q) = ', min(data_full[indx_z_cut_full03z25].M_I)
;print, 'max Ab. i-Mag (full DR5Q) = ', max(data_full[indx_z_cut_full03z25].M_I)
;print
indx_z_cut_full22 = where(data_full.z ge 0.30 and data_full.z le 2.20, N_z_cut)
...
indx_z_cut25 = where(data.z le 2.50, N_z_cut)
print, 'UNIFORM, N_z_cut, z<2.5', N_z_cut
print, 'minimun redshift (homog, z<2.5) = ', min(data[indx_z_cut25].z)
print, 'maximum redshift (homog, z<2.5) = ', max(data[indx_z_cut25].z)
print, 'min Ab. i-Mag (Shen) = ', min(data[indx_z_cut29].imag)
print, 'max Ab. i-Mag (Shen) = ', max(data[indx_z_cut29].imag)
print
...
indx_z_cut03z25 = where(data.z ge 0.30 and data.z le 2.50, N_z_cut)
print, 'UNIFORM, N_z_cut 0.30 _< z _< 2.50', N_z_cut
print, 'min  redshift (homog, 0.3_<z_<2.5) = ', min(data[indx_z_cut03z25].z)
print, 'max  redshift (homog, 0.3_<z_<2.5) = ', max(data[indx_z_cut03z25].z)
print, 'min Ab. i-Mag (Shen) = ', min(data[indx_z_cut03z25].imag)
print, 'max Ab. i-Mag (Shen) = ', max(data[indx_z_cut03z25].imag)
print
print
print
...
print
print, 'PRIMARY, N_z_cut_full 0.30 _< z _< 2.5', N_TS_T_PRIz22
...
plothist, data_full.z, bin=0.05, $
          ...
          xtitle='redshift, z', ytitle='Number of obecjts (0.05 bins)'
plothist, data_full[indx_PRIz].z,  bin=0.05, /overplot, linestyle=0, color=64, font=-2, thick=1
plothist, data_full[indx_PRIz22].z,  bin=0.05, /overplot, linestyle=0, color=64, font=-2, thick=6
plothist, data[indx_z_cut03z29].z,      bin=0.05, /overplot, linestyle=2, color=192, font=-2, thick=1
plothist, data[indx_z_cut03z22].z,      bin=0.05, /overplot, linestyle=2, color=192, font=-2, thick=4
...
legend,['DR5Q','PRIMARY','UNIFORM'],line=[1,0,2], box=0, position=[2.0,2250], color=[0,64,192],charsize=1.8, charthick=4.2
...
plot, data_full[indx_PRIz].z, $
;plot, data_full[indx_HIZz].z, $
;          xrange=[-0.1, 3.0],  yrange=[0,799], $ ; for HIZz
          xrange=[0.1, 3.0],  yrange=[0,1800], $ ; for PRIz
          xstyle=1, ystyle=1, $
