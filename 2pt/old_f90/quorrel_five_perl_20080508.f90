...
allocate(r_c(45000), z_rnd_bin(101))
...
open(27,file='K_wp_output_temp.dat')
...
do while (istat .eq. 0) 
   ...
   if((ra_DR5Q(i) .ge. ra_min) .and. (ra_DR5Q(i) .le. ra_max)) then
      if((dec_DR5Q(i) .ge. dec_min) .and. (dec_DR5Q(i) .le. dec_max)) then
         if((z_DR5Q(i) .ge. z_min) .and. (z_DR5Q(i) .le. z_max)) then
            ...
            if(d .ge. 1 .and. d .le. 101) z_bin(d) = z_bin(d) + 1
...
do while (istat .eq. 0) 
   ...
   if((ra_rnd_npr (i) .ge. ra_min) .and. (ra_rnd_npr(i) .le. ra_max)) then
      if((dec_rnd_npr(i) .ge. dec_min) .and. (dec_rnd_npr(i) .le. dec_max)) then
         if((z_rnd_npr(i) .ge. z_min) .and. (z_rnd_npr(i) .le. z_max)) then
            ...
            if(d .ge. 1 .and. d .le. 101) z_rnd_bin(d) = z_rnd_bin(d) + 1
...
write(*,*) 'Read-in random points', N_rnd, 'equals', (N_rnd/660220.)*100.,'%'
...
write(*,*)
write(*,*) '(float(k)-12+0.5)/10., 10**((float(k)-12+0.5)/10.),  bin_DR(k)'
   ...
   !do k_pi_para =1,41
      ...
      if(bin_DR_2d(k_sigma,k_pi_para) .ne. 0 .and. &
         ...
         if(k_pi_para .le. 20) then 
            !19 => Putting a pi_para cut in at 31 h^-1 Mpc, 5 bins per decade
...
open(44,file='z_values.dat')
write(*,*) 'almost there..'
write(*,*) 'z_max', z_max
write(*,*)
if( choice_two .ne. 8 .or. choice_two .ne. 9) z_max = 1.1
z_max = int(z_max * zbin_width) + 2
write(*,*) z_max
!do i = z_max,1,-1
do i = 111,1,-1
   hundred_z = real( i / zbin_width)  
   cumulate = cumulate + int(z_bin(i))
!   write(*,*) 'z_max', z_max,  'zbin_width', zbin_width
!   write(*,*) 'i', i, 'hundered_z', hundred_z
!   write(*,1600) hundred_z, int(z_bin(i))  
!   write(*,*) 'i, z_max, zbin_width' 
!   write(*,*) i, z_max, zbin_width, 
!   write(*,*)
!  write(44,1600) hundred_z, int(z_bin(i)), real(z_bin(i)/N_data), (real(cumulate)/(N_data-N_star))
!write(44,1601) hundred_z, int(z_bin(i)), int(z_rnd_bin(i)), &
!     & real(z_bin(i)/N_data), (real(cumulate)/(N_data-N_star))
end do
