...
real*8:: ra_SDSS2, dec_SDSS2,  Landy2d,  Szalay2d, dec_min, dec_max
...
n = 1000000
m = 70000 !This could well need to be increased...
N_lns=0
N_data = 0
N_rnd = 0
N_star=0
...
allocate(x_coord(45000), y_coord(45000), z_coord(45000))
allocate(x_coord_rnd(n), y_coord_rnd(n), z_coord_rnd(n))
allocate(r_c(45000), r_c_rnd(n), z_rnd_bin(101), ra_theta(m), dec_theta(m))
...
do while (istat .eq. 0) 
   ...
   read(1,*,IOSTAT=istat) x_coord(N_LRG_sample), y_coord(N_LRG_sample), &
        & z_coord(N_LRG_sample) !,  &
   !    & ra_theta(N_LRG_sample, dec_theta(N_LRG_sample)
   ! AT SOME POINT HAVE TO INCLUDE ra_theta, dec_theta for Fibre collisions..
   !
