...
;for i = 0, 173  do begin
 ...
 ;  if i MOD 8656 eq 0 then print, ( (double(i) / 173120.)*100.), '% RRs done'
   if i MOD 10000 eq 0 then print, ( (double(i) / 1000000.)*100.), '% RRs done'
...
;   delta_xi(k) = (1. + xi(k)) * (sqrt(2.0/bin_DR(k)))
   ...
   printf, 9, k, 10^((float(k)-15.+0.5)/10.), h_DD(k), bin_DR(k), h_RR(k), $
           xi(k), xi_STD(k), xi_LS(k), $
           format='(i,d, i,i,i, d,d,d)'
endfor
...
printf, 18, ' # k_sigma, k_pi_para, 10^((float(k_sigma)-15.+0.5)/10.), 10^((float(k_pi_para)-15.+0.5)/10.), h_DD_2d, bin_DR_2d, h_RR_2d, xi, xi_STD, xi_HAM, xi_LS'
...
bin_sigma_pit_HAM  =  fltarr(70,70)
...
;      if k_pi_para le 19 then begin ; pi_cut = 63.0 h^-1 Mpc if + 12
      bin_sigma_pit(k_sigma,k_pi_para)    = xi_sigma_pit(k_sigma,k_pi_para) $
                                           *(10^ ( (float(k_pi_para)-14)/10.) $
                                           - 10^ ( (float(k_pi_para)-15)/10.))

     bin_sigma_pit_HAM(k_sigma,k_pi_para) = xi_sigma_pit_HAM(k_sigma,k_pi_para) $
                                           *(10^ ( (float(k_pi_para)-14)/10.) $
                                           - 10^ ( (float(k_pi_para)-15)/10.))
      
      bin_sigma_pit_LS(k_sigma,k_pi_para) = xi_sigma_pit_LS(k_sigma,k_pi_para) $
                                           *(10^ ( (float(k_pi_para-14))/10.) $
                                           - 10^ ( (float(k_pi_para-15))/10.))
      endif else begin
         bin_sigma_pit(k_sigma,k_pi_para)     = 0.0
         bin_sigma_pit_HAM(k_sigma,k_pi_para) = 0.0
         bin_sigma_pit_LS(k_sigma,k_pi_para)  = 0.0
      endelse

      
      Xi_sigmat(k_sigma)      = Xi_sigmat(k_sigma) + $
                             (2.*bin_sigma_pit(k_sigma,k_pi_para)) 
      Xi_sigmat_HAM(k_sigma)  = Xi_sigmat_HAM(k_sigma) + $
                             (2.*bin_sigma_pit_HAM(k_sigma,k_pi_para)) 
      Xi_sigmat_LS(k_sigma)   = Xi_sigmat_LS(k_sigma) + $
                             (2.*bin_sigma_pit_LS(k_sigma,k_pi_para)) 
      
    bin_DD_sigmat(k_sigma)=bin_DD_sigmat(k_sigma)+(h_DD_2dt(k_sigma,k_pi_para))
    bin_DR_sigmat(k_sigma)=bin_DR_sigmat(k_sigma)+(bin_DR_2dt(k_sigma,k_pi_para))



      printf, 19, k_sigma, k_pi_para, $
          10^((float(k_sigma)-15+0.5)/10.), 10^((float(k_pi_para)-15+0.5)/10.), $
              h_DD_2dt(k_sigma,k_pi_para), bin_DR_2dt(k_sigma,k_pi_para), $
              h_RR_2dt(k_sigma,k_pi_para), $
              xi_sigma_pit(k_sigma,k_pi_para), $
              xi_sigma_pit_STD(k_sigma,k_pi_para), $ 
FIX ME HERE!!, line 833, delta_xi_sigmat....
              xi_sigma_pit_HAM(k_sigma,k_pi_para), $
              xi_sigma_pit_LS(k_sigma,k_pi_para), $ 
              format='(i,1x,i,1x,  d11.5,2x,d11.5,2x,  d15.1,1x,d15.1,1x,d15.1,1x,  d15.6,1x,d15.6,1x,d15.6,1x,d15.6)'
      
   endfor  ;k_pi_para =0, 21  do begin

   
   if (bin_DD_sigmat(k_sigma) ne 0. and bin_DR_sigmat(k_sigma) ne 0.) then begin
      delta_Xi_sigmat(k_sigma) = (1 + Xi_sigmat(k_sigma)) $
                                * (sqrt(2.0 / bin_DD_sigmat(k_sigma)))
   endif else begin
      delta_Xi_sigmat(k_sigma) = 0.
   endelse

   printf, 28, 10^((float(k_sigma)-15+0.5)/10.), (float(k_sigma)-15+0.5)/10., $
           Xi_sigmat(k_sigma), delta_Xi_sigmat(k_sigma), $
           bin_DD_sigmat(k_sigma),bin_DR_sigmat(k_sigma), $
           Xi_sigmat_HAM(k_sigma), Xi_sigmat_LS(k_sigma), $
           format='(d11.6,1x,d7.3,2x,  d16.8,1x,d16.8,2x,  d16.2,1x,d16.2,2x, d16.4,1x,d16.4)' 
