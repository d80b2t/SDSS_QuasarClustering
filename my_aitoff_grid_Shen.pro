...
PRO MY_AITOFF_GRID,DLONG,DLAT,LABEL=LABEL, NEW = new, _EXTRA= E, $
     CHARSIZE = charsize, CHARTHICK =charthick, color = color, iplot = iplot, offset = offset
...
;
          if (!d.name eq 'PS') and (!p.font eq 0) then hr = '!Uh!N' else hr='h'
            ...
            for i=0,2*lngtot-1 do begin
                ...
                if (lng ne 150.0) and (lng ne  90.0) and (lng ne 30.0) and $
                   (lng ne 330.0) and (lng ne 270.0) and (lng ne 210.0) then begin
                    aitoff,lng,0.0,x,y
                    if label eq 1 then xyouts,x[0]+5,y[0]-15,$
                        strcompress(string((lng+offset) mod 360,format="(I4)"),/remove_all), $
                        charsize = charsize, charthick = charthick, color = fsc_color('black') $
                    else begin
                         tmp = lng/15.
