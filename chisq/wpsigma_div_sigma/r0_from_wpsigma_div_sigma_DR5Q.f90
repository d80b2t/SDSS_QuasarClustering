...
if(choice.eq.4) then
   ...
   elseif(choice_two .eq.1) then
      open(10,file='../../OP/UNI22_evol_wpsigma/rv2/K_wp_output_UNI22_0pnt30z0pnt68_rv2.dat')
      write(*,*) 'OP/UNI22_evol_wpsigma/rv2/K_wp_output_UNI22_0pnt30z0pnt68_rv2.dat'
   elseif(choice_two .eq.2) then
      open(10,file='../../OP/UNI22_evol_wpsigma/rv2/K_wp_output_UNI22_0pnt68z0pnt92_rv2.dat')
      write(*,*) 'OP/UNI22_evol_wpsigma/rv2/K_wp_output_UNI22_0pnt68z0pnt92_rv2.dat'
   elseif(choice_two .eq.3) then
      open(10,file='../../OP/UNI22_evol_wpsigma/rv2/K_wp_output_UNI22_0pnt92z1pnt13_rv2.dat')
      write(*,*) 'OP/UNI22_evol_wpsigma/rv2/K_wp_output_UNI22_0pnt92z1pnt13_rv2.dat'
   elseif(choice_two .eq.4) then
      open(10,file='../../OP/UNI22_evol_wpsigma/rv2/K_wp_output_UNI22_1pnt13z1pnt32_rv2.dat')
      write(*,*) 'OP/UNI22_evol_wpsigma/rv2/K_wp_output_UNI22_1pnt13z1pnt32_rv2.dat'
   elseif(choice_two .eq.5) then
      open(10,file='../../OP/UNI22_evol_wpsigma/rv2/K_wp_output_UNI22_1pnt32z1pnt50_rv2.dat')
      write(*,*) 'OP/OP_20080215/K_wp_output_20080121_1pnt32z1pnt50_rv2.dat'
   elseif(choice_two .eq.6) then 
      open(10,file='../../OP/UNI22_evol_wpsigma/rv2/K_wp_output_UNI22_1pnt50z1pnt66_rv2.dat')
      write(*,*) 'OP/UNI22_evol_wpsigma/rv2/K_wp_output_UNI22_1pnt50z1pnt66_rv2.dat'
   elseif(choice_two .eq.7) then
      open(10,file='../../OP/UNI22_evol_wpsigma/rv2/K_wp_output_UNI22_1pnt66z1pnt83_rv2.dat')
      write(*,*) 'OP/UNI22_evol_wpsigma/rv2/K_wp_output_UNI22_1pnt66z1pnt83_rv2.dat'
   elseif(choice_two .eq.8) then
      open(10,file='../../OP/UNI22_evol_wpsigma/rv2/K_wp_output_UNI22_1pnt83z2pnt02_rv2.dat')
      write(*,*) 'OP/UNI22_evol_wpsigma/rv2/K_wp_output_UNI22_1pnt83z2pnt02_rv2.dat'
   elseif(choice_two .eq.9) then
      open(10,file='../../OP/UNI22_evol_wpsigma/rv2/K_wp_output_UNI22_2pnt02z2pnt20_rv2.dat')
      write(*,*) 'OP/UNI22_evol_wpsigma/rv2/K_wp_output_UNI22_2pnt02z2pnt20_rv2.dat'
   elseif(choice_two .eq. 10) then
      open(10,file='../../OP/UNI22_evol_wpsigma/rv2/K_wp_output_UNI22_2pnt20z2pnt90_rv2.dat')
      write(*,*) 'OP/UNI22_evol_wpsigma/rv2/K_wp_output_UNI22_2pnt20z2pnt90_rv2.dat'
   elseif(choice_two .eq. 11) then
      open(10,file='../../OP/UNI22_evol_wpsigma/rv2/K_wp_output_UNI22_0pnt00z0pnt30_rv2.dat')
      write(*,*) 'OP/UNI22_evol_wpsigma/rv2/K_wp_output_UNI22_0pnt00z0pnt30_rv2.dat'
   endif
endif   
...
do while(istat .eq. 0)
   ...
   if(choice .eq. 4) then 
      ...
      if (choice_two .eq. 0) then
         ...
         !! error using the ``standard'' estimator and the wp(rp) DDs 
         !! (from quorrel_five_perl.f90
      ...
      elseif(choice_two .ne. 0) then
         read(10,*,IOSTAT=istat) s(i), lg_s(i), xis(i),  delta_xis(i), &
              & bin_DD_sigma(i), bin_DR_sigma(i),  xi_HAM(i), xis_LS(i)
         jack_err(i)           = delta_xis(i)
         jack_err_div_sigma(i) = jack_err(i)  / s(i) 
!         if (xis_LS(i) .ne. 0.) n_input = n_input +1 
      endif
      
      write(*,*) i, s(i), lg_s(i), xis(i),  delta_xis(i), bin_DD_sigma(i), &
           & bin_DR_sigma(i), xi_HAM(i), xis_LS(i), jack_err(i), jack_err_div_sigma(i)
      
   endif
end do
n_input = i - 1 
write(*,*)
write(*,*) 'Read-in', i,' with ', n_input,' having non-zero wp(rp)'
write(*,*)
...
!! 3.1622777 0.5 -4.707458979053209 3.838188939609616 14. 711. -10.886927728047638 -13.735350929340107
...
!! Looping over values for r0, holding gamma fixed
do j=1,3000
   
   !s_0vec(j) = 2.0+float(j-1)*0.05
   s_0vec(j) = 3.0+float(j-1)*0.005
   s0 = s_0vec(j)
   
   do k =1,1   
      gamma = gamma_fixed
      !! fixed at gamma=2 from above
      ...
      do i=1,n_input
         ...
         if (jack_err(i) .gt. 0.0 .and. s(i).ge.s_lo  .and. s(i).le.s_hi) then
            ...
            !! For wp(rp) / rp
            !! Note this is setting up w(rp)/rp 
            !! see e.g. Eqn. (6), Hawkins et al. (2003)
            xi_s_test(i) = (s0**gamma) * (s(i)**(-gamma)) * A
            
            chi2(j,k) = chi2(j,k) + ((( xi_s_test(i)- (xis_LS(i)/s(i)))**2.) /  &
                 & (jack_err_div_sigma(i)*jack_err_div_sigma(i)))
            
            write(21,21) i,j,k, s0, s(i), xi_s_test(i), xis_LS(i), xis_LS(i)/s(i), jack_err_div_sigma(i), &
                 & chi2(j,k), chi_sq_min
            count = count+1
