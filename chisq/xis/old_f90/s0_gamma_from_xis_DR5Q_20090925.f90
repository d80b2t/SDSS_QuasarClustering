...
real*8, allocatable:: wp_sigma_measured(:), wp_sigma(:)
...
real*8, allocatable:: bin_DD_sigma(:), bin_DR_sigma(:)
...
allocate(bin_DD_sigma(m), bin_DR_sigma(m))
allocate(bin_DR(m), xis_LS(m), xi_HAM(m), jack_err(m), wp_ratio_correction(m))
allocate(wp_sigma_measured(m), wp_sigma(m))
...
if(choice.eq.1) then      !xi(s) 
   ...
   if (choice_two .eq. 0) then
      write(*,*) 'With Fibre Collision corrections?  y/n  '
      read(*,*) fibre_on_off
      if (fibre_on_off .eq. 'n') then
         open(10,file='../../OP/OP_20080508/k_output_UNI22.dat')
         write(*,*) 'OP/20080508/k_output_UNI22.dat'
      end if
      if (fibre_on_off .eq. 'y') then
         open(10,file='../../OP/OP_UNI22_wFibre/k_output_UNI22_wFibre.dat')
         write(*,*) 'OP/20080508/k_output_UNI22.dat'
      endif
...
if(choice .eq. 3) then 
   write(*,*) 
   if (choice_two .eq. 0) then
      open(10,file='OP/OP_20080120/K_wp_output_20080120_0pnt30z2pnt90.dat')
      write(*,*) 'OP/OP_20080120/K_wp_output_20080120_0pnt30z2pnt90.dat'
   elseif(choice_two .eq.1) then
...
if(choice .eq. 3) open(20, file='wp_sigma_DR5Q_chisq.dat', status='unknown')  
...
do while(istat .eq. 0)
   ...
   elseif(choice .eq. 3) then 
      read(10,*,IOSTAT=istat) s(i), lg_s(i), xis(i),  delta_xis(i), &
           &  bin_DD_sigma(i), bin_DR_sigma(i), xi_HAM(i), xis_LS(i)
      !read(10,*,IOSTAT=istat)  s(i), lg_s(i), Xi_sigma, delta_Xi_sigma
